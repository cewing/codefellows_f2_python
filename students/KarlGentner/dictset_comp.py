...
print u"\nOne liner- Dictionary 'z' with keys 0 to 15 and hexadecimal equivalent values:"
displayDict(z)
...
def divSetGen(n, divRange):
    """Return a list of 'n' sets that each contains numbers from 0 to 'divRange', divisible by the list index + 2 """
    return [{i for i in range(divRange+1) if i % (x+2) == 0} for x in range(n)]

lsfive = divSetGen(5, 20)
print u"\n\n My own add-on: Function returns - List with sets s2, s3, s4, s5, and s6:"
